...
module.exports = async () => {
  ...
  if (process.env.BFX_CLI_CREDENTIALS_LOADED) {
    ...
    try {
      ...
      log('Deleted ~/%s', RC_FILENAME)
    } catch (e) {
      log('Failed to delete ~/%s: %s', RC_FILENAME.red, e)
    }
  ...
  prompt = new Confirm({
    ...
    message: 'Do you want to encrypt your credentials with a password? You will be prompted for the password every time you run bfx-cli:'
  })
  ...
  try {
    ...
    log('Failed to save credentials: %s', e)
  }
